# html_generator.py
# Модуль генерации HTML-файла с визуализацией дерева технологий

import json
from config import HTML_OUTPUT, START_TECH


def generate_html_template():
    """Возвращает шаблон HTML без данных"""
    return """<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <style>
        ...
        .card-stats { display: flex; gap: 15px; margin: 10px 0; padding: 8px; background: #f8f9fa; border-radius: 4px; font-size: 0.9rem; }
        .stat-item { display: flex; flex-direction: column; }
        .stat-label { color: #6c757d; font-size: 0.75rem; text-transform: uppercase; }
        .stat-value { font-weight: bold; color: #212529; }
        .stat-value.cost { color: #dc3545; }
        .stat-value.points { color: #28a745; }
...
<body>
    ...
    <script>
        ...
        const ui_lexicon = {
            en: { 
                title: "X-Piratez Viewer", 
                depth: "Depth: ", 
                hub: "Max List: ", 
                rebuild: "REBUILD GRAPH", 
                req: "Requires ALL (AND)", 
                unl: "Unlocked by Any (OR)", 
                back: "← BACK",
                cost: "Cost",
            ...
            ru: { 
                title: "X-Piratez Навигатор", 
                depth: "Глубина: ", 
                hub: "Макс. список: ", 
                rebuild: "ПЕРЕСТРОИТЬ ГРАФ", 
                req: "Требует ВСЕ (AND)", 
                unl: "Открывается через (OR)", 
                back: "← НАЗАД",
                cost: "Стоимость",
                points: "Очки исследования",
                disables: "Отключает"
            }
        };
        ...
        function showDetail(id) {
            ...
            // Display cost and points statistics
            const techData = rawData[id];
            h += "<div class='card-stats'>";
            h += "<div class='stat-item'><span class='stat-label'>"+lex.cost+"</span><span class='stat-value cost'>"+techData.cost+"</span></div>";
            h += "<div class='stat-item'><span class='stat-label'>"+lex.points+"</span><span class='stat-value points'>"+techData.points+"</span></div>";
            h += "</div>";
            ...
            // Display disables if present
            const disables = rawData[id].disables;
            if(disables && disables.length > 0) {
                h += "<div class='card'><b class='or-label' style='color:#dc3545;'>"+lex.disables+" ("+disables.length+"):</b>"+fmt(disables)+"</div>";
            }
...
def build_language_options(langs_data):
    """Создаёт HTML-опции для выбора языка"""
    options = []
    for code in langs_data.keys():
        selected = 'selected' if code == 'en-US' else ''
        options.append(f'<option value="{code}" {selected}>{code}</option>')
    return "".join(options)


def generate_html_file(raw_data, game_langs_data, output_path=None, start_tech=None):
    """
    Генерация итогового HTML-файла
    
    Args:
        raw_data: Словарь с данными технологий
        game_langs_data: Словарь с языковыми данными
        output_path: Путь для сохранения (по умолчанию из config)
        start_tech: ID стартовой технологии (по умолчанию из config)
    
    Returns:
        Путь к созданному файлу
    """
    if output_path is None:
        from config import HTML_OUTPUT as output_path
    if start_tech is None:
        from config import START_TECH as start_tech
    
    # Сериализация данных в JSON
    data_json = json.dumps(raw_data, ensure_ascii=False)
    langs_json = json.dumps(game_langs_data, ensure_ascii=False)
    
    # Построение опций языков
    lang_options = build_language_options(game_langs_data)
    
    # Загрузка шаблона и подстановка данных
    html_template = generate_html_template()
    html = html_template.replace('__OPTIONS__', lang_options)\
                       .replace('__DATA__', data_json)\
                       .replace('__LANGS__', langs_json)\
                       .replace('__START_TID__', start_tech)
...
if __name__ == "__main__":
    # Test HTML generator with sample data including cost and points
    print("Testing HTML generator...")
    test_raw = {
        "STR_TEST1": {"deps": ["STR_TEST2"], "unlocks": ["STR_TEST3"], "disables": [], "cost": 50, "points": 200},
        "STR_TEST2": {"deps": [], "unlocks": ["STR_TEST1"], "disables": [], "cost": 25, "points": -100},
        "STR_TEST3": {"deps": ["STR_TEST1"], "unlocks": [], "disables": ["STR_TEST4"], "cost": 75, "points": 500},
        "STR_TEST4": {"deps": [], "unlocks": [], "disables": [], "cost": 10, "points": 50}
    }
    test_langs = {
        "en-US": [{"STR_TEST1": "Test Tech 1", "STR_TEST2": "Test Tech 2", "STR_TEST3": "Test Tech 3", "STR_TEST4": "Test Tech 4"}, {}],
        "ru": [{"STR_TEST1": "Тест Технология 1", "STR_TEST2": "Тест Технология 2", "STR_TEST3": "Тест Технология 3", "STR_TEST4": "Тест Технология 4"}, {}]
    }
    
    output = generate_html_file(test_raw, test_langs, output_path="test_viewer.html", start_tech="STR_TEST1")
    print(f"Test file created: {output}")
